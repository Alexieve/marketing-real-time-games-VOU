    ...
    const fetchData = async () => {
      try {
        ...
        response = await fetch(
          `${localhost}/api/game/customer-item/?customerID=${user.id}&eventID=${eventID}`
        );
              ...
              eventVouchers.map((voucher, index) => {
                ...
                return (
                  <Card key={voucher._id} containerStyle={styles.voucherCard}>
                    ...
                    <Text style={styles.voucherDetail}>
                      Expired: {new Date(voucher.expTime).toLocaleString()}
                    </Text>
                    {gameID == "1" && (
                      <Button
                        ...
                        onPress={() =>
                          handleHQExchange(voucher._id, voucher.price, index)
                        }
                        icon={
                          <Icon
