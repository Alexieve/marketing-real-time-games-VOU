...
const VoucherEdit = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`/api/event_command/voucher_detail/${id}`)
      .then(async function (response) {
        let time = response.data.expTime;
        let localTime = new Date(new Date(time) - 7 * 3600 * 1000);
        response.data.expTime = moment(localTime).format("YYYY-MM-DDTHH:mm");
        ...
        const file = new File([blob], response.data.imageUrl.split("/").pop(), {
          type: blob.type,
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `/api/event_command/voucher/update/${id}`,
        formData,
        {
          headers: {
                  ...
                  <CForm onSubmit={handleSubmit}>
                    <CInputGroup className="mb-3">
                      ...
                      <CFormInput
                        ...
                        required
                        disabled={
                          new Date(voucherData.expTime) < new Date()
                            ? true
                            : false
                        }
                      />
